...
public class ScanPsyva implements IConst {
	...
	public void scanSummary(boolean fatalErr) {
		...
		if (synchk.isValidPsyva()) {
			out("Psyva file is valid.");
		}
		else {
			out("Psyva file is invalid!");
		}
	}
	...
	private TokenTyp getTokLowerCase(String token) {
		...
		PsyvaFnTyp cftyp = PsyvaFnTyp.NULL;
		...
		if (isKeyword) {
			...
			isKeyword = true;
			try {
				cftyp = PsyvaFnTyp.valueOf(token);  // cftyp != zparen
			} catch (IllegalArgumentException exc) {
				isKeyword = false;
			}
			if (isKeyword) {
	...
	private void putFun(String token, PsyvaFnTyp cftyp) {
		String outbuf;
		char sp = ' ';
