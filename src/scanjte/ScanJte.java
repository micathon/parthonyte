...
public class ScanJte implements IConst {
	...
	public void scanSummary(boolean fatalErr) {
		...
		if (synchk.isValidJte()) {
			out("Jte file is valid.");
		}
		else {
			out("Jte file is invalid!");
		}
	}
	...
	private TokenTyp getTokLowerCase(String token) {
		...
		if (isKeyword) {
			...
			isKeyword = true;
			try {
				cftyp = JteFnTyp.valueOf(token);  // cftyp != zparen
			} catch (IllegalArgumentException exc) {
				isKeyword = false;
			}
			if (isKeyword) {
	...
	private void putFun(String token, JteFnTyp cftyp) {
		String outbuf;
		char sp = ' ';
