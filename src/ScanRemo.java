...
public class ScanRemo implements IConst {
	...
	public void scanSummary(boolean fatalErr) {
		...
		if (synchk.isValidRemo()) {
			out("Remo file is valid.");
		}
		else {
			out("Remo file is invalid!");
		}
	}
	...
	private TokenTyp getTokLowerCase(String token) {
		...
		RemoFnTyp cftyp = RemoFnTyp.NULL;
		...
		if (isKeyword) {
			...
			isKeyword = true;
			try {
				cftyp = RemoFnTyp.valueOf(token);  // cftyp != zparen
			} catch (IllegalArgumentException exc) {
				isKeyword = false;
			}
			if (isKeyword) {
	...
	private void putFun(String token, RemoFnTyp cftyp) {
		String outbuf;
		char sp = ' ';
