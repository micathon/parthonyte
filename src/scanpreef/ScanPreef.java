...
public class ScanPreef implements IConst {
	...
	public void scanSummary(boolean fatalErr) {
		...
		if (synchk.isValidPreef()) {
			out("Preef file is valid.");
		}
		else {
			out("Preef file is invalid!");
		}
	}
	...
	private TokenTyp getTokLowerCase(String token) {
		...
		PreefFnTyp cftyp = PreefFnTyp.NULL;
		...
		if (isKeyword) {
			...
			isKeyword = true;
			try {
				cftyp = PreefFnTyp.valueOf(token);  // cftyp != zparen
			} catch (IllegalArgumentException exc) {
				isKeyword = false;
			}
			if (isKeyword) {
	...
	private void putFun(String token, PreefFnTyp cftyp) {
		String outbuf;
		char sp = ' ';
