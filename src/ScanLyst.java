...
public class ScanLyst implements IConst {
	...
	public void scanSummary(boolean fatalErr) {
		...
		if (synchk.isValidLyst()) {
			out("Lyst file is valid.");
		}
		else {
			out("Lyst file is invalid!");
		}
	}
	...
	private TokenTyp getTokLowerCase(String token) {
		...
		LystFnTyp cftyp = LystFnTyp.NULL;
		...
		if (isKeyword) {
			...
			isKeyword = true;
			try {
				cftyp = LystFnTyp.valueOf(token);  // cftyp != zparen
			} catch (IllegalArgumentException exc) {
				isKeyword = false;
			}
			if (isKeyword) {
	...
	private void putFun(String token, LystFnTyp cftyp) {
		String outbuf;
		char sp = ' ';
