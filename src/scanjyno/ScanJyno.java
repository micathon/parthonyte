...
public class ScanJyno implements IConst {
	...
	public void scanSummary(boolean fatalErr) {
		...
		if (synchk.isValidJyno()) {
			out("Jyno file is valid.");
		}
		else {
			out("Jyno file is invalid!");
		}
	}
	...
	private TokenTyp getTokLowerCase(String token) {
		...
		JynoFnTyp cftyp = JynoFnTyp.NULL;
		...
		if (isKeyword) {
			...
			isKeyword = true;
			try {
				cftyp = JynoFnTyp.valueOf(token);  // cftyp != zparen
			} catch (IllegalArgumentException exc) {
				isKeyword = false;
			}
			if (isKeyword) {
	...
	private void putFun(String token, JynoFnTyp cftyp) {
		String outbuf;
		char sp = ' ';
