...
public class ScanCoop implements IConst {
	...
	public void scanSummary(boolean fatalErr) {
		...
		if (synchk.isValidCoop()) {
			out("Coop file is valid.");
		}
		else {
			out("Coop file is invalid!");
		}
	}
	...
	private TokenTyp getTokLowerCase(String token) {
		...
		CoopFnTyp cftyp = CoopFnTyp.NULL;
		...
		if (isKeyword) {
			...
			isKeyword = true;
			try {
				cftyp = CoopFnTyp.valueOf(token);  // cftyp != zparen
			} catch (IllegalArgumentException exc) {
				isKeyword = false;
			}
			if (isKeyword) {
	...
	private void putFun(String token, CoopFnTyp cftyp) {
		String outbuf;
		char sp = ' ';
